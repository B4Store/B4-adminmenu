...
RegisterNetEvent('B4-admin:client:openMenu', function()
    MenuV:OpenMenu(menu)
end)
...
menu_button11:On("select",function()
    ...
    for _, v in ipairs(elements) do
        menu6:AddButton({icon = v.icon,label = v.label,value = v,description = v.description,select = function(btn)
            ...
            TriggerServerEvent('B4-weathersync:server:setWeather', selection.value)
...
menu_dev_button:On('change', function(_, _, _)
    dev = not dev
    TriggerEvent('B4-admin:client:ToggleDevmode')
    if dev then
        while dev do
            Wait(200)
...
RegisterNetEvent('B4-admin:client:copyToClipboard', function(dataType)
...
RegisterNetEvent('B4-admin:client:ToggleCoords', function()
...
names_button:On('change', function()
    TriggerEvent('B4-admin:client:toggleNames')
end)
blips_button:On('change', function()
    TriggerEvent('B4-admin:client:toggleBlips')
end)
...
for _,v in pairs(QBCore.Shared.Weapons) do
  menu16:AddButton({icon = '🔫',
                    ...
                    description = Lang:t("desc.spawn_weapons_desc"),
                    select = function(_)
        TriggerServerEvent('B4-admin:giveWeapon', v.name)
...
local function OpenPermsMenu(permsply)
    QBCore.Functions.TriggerCallback('B4-admin:server:getrank', function(rank)
        if rank then
            ...
            menu10:AddButton({
                ...
                select = function(_)
                    if selectedgroup ~= 'Unknown' then
                        TriggerServerEvent('B4-admin:server:setPermissions', permsply.id, selectedgroup)
    ...
    menu9:AddButton({
        ...
        select = function(_)
            if kickreason ~= 'Unknown' then
                TriggerServerEvent('B4-admin:server:kick', kickplayer, kickreason)
                kickreason = 'Unknown'
            else
    ...
    menu8:AddButton({
        ...
        select = function(_)
            if banreason ~= 'Unknown' and banlength ~= nil then
                TriggerServerEvent('B4-admin:server:ban', banplayer, banlength, banreason)
                banreason = 'Unknown'
                banlength = nil
            else
        ...
        Players:AddButton({
            ...
            select = function(btn)
                ...
                if values ~= "ban" and values ~= "kick" and values ~= "perms" then
                    TriggerServerEvent('B4-admin:server:'..values, player)
...
menu_button13:On("select", function(_, value)
    TriggerServerEvent("B4-weathersync:server:setTime", value, value)
