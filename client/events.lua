...
RegisterNetEvent('B4-admin:client:inventory', function(targetPed)
    TriggerServerEvent("inventory:server:OpenInventory", "otherplayer", targetPed)
end)

RegisterNetEvent('B4-admin:client:spectate', function(targetPed)
...
RegisterNetEvent('B4-admin:client:SendReport', function(name, src, msg)
    TriggerServerEvent('B4-admin:server:SendReport', name, src, msg)
end)

RegisterNetEvent('B4-admin:client:SendStaffChat', function(name, msg)
    TriggerServerEvent('B4-admin:server:Staffchat:addMessage', name, msg)
end)
...
RegisterNetEvent('B4-admin:client:SaveCar', function()
    ...
    if veh ~= nil and veh ~= 0 then
        ...
        if QBCore.Shared.Vehicles[vehname] ~= nil and next(QBCore.Shared.Vehicles[vehname]) ~= nil then
            TriggerServerEvent('B4-admin:server:SaveCar', props, QBCore.Shared.Vehicles[vehname], GetHashKey(veh), plate)
        else
...
RegisterNetEvent('B4-admin:client:SetModel', function(skin)
    local ped = PlayerPedId()
...
RegisterNetEvent('B4-admin:client:SetSpeed', function(speed)
    local ped = PlayerId()
    if speed == "fast" then
...
RegisterNetEvent('B4-admin:client:GiveNuiFocus', function(focus, mouse)
...
RegisterNetEvent('B4-admin:client:maxmodVehicle', function()
